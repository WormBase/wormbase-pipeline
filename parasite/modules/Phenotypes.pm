...
sub generate_phenotype_gff {
  ...
  my $sth = $pfa->dbc->prepare(qq{
    SELECT
      ...
      IF(gn.display_xref_id IS NULL,pf.object_id,xr.display_label) AS object_symbol,
      IF(p.description LIKE "%no phenotype observed","NOT","") AS qualifier,
      poa.accession,
      ...
      FROM
        ...
        study st,
        phenotype p,
        ensembl_metadata_$ENV{'ENSEMBL_VERSION'}.data_release edr,
        phenotype_feature pf

      LEFT JOIN phenotype_feature_attrib pfa
        ON (pf.phenotype_feature_id = pfa.phenotype_feature_id)
      LEFT JOIN phenotype_ontology_accession poa
        ON (pf.phenotype_id = poa.phenotype_id)
      ...
      LEFT JOIN phenotype_feature_attrib pfa2
        ON (pf.phenotype_feature_id = pfa2.phenotype_feature_id)
      LEFT JOIN $config->{cdbname}.gene gn
        ON (pf.object_id = gn.stable_id)
      LEFT JOIN $config->{cdbname}.xref xr
        ON (gn.display_xref_id = xr.xref_id)

      WHERE sr.seq_region_id = pf.seq_region_id
      AND s.source_id = pf.source_id
      AND pf.phenotype_id = p.phenotype_id
      AND pfa2.attrib_type_id = (SELECT attrib_type_id FROM attrib_type WHERE code="external_db")
      AND pf.study_id = st.study_id
  ...
  print STDERR "### Phenotypes plugin: Gzipping file\n" unless $config->{quiet};

  system("gzip $file;") and die("ERROR: gzipping failed\n");
