...
foreach (@$data){
	...
	if($ref_proteome){
        ...
        chdir "tmp";
		if(!-e "$ref_proteome.json"){
			`wget $uniprot_api/$ref_proteome.json`;
		}
		my $uniprot_json = JSON->new;
		open my $UNIPROT_FH, "<", "$ref_proteome.json" or die "Could not open uniprot file \n";
		...
		if($genome){
			...
			if ( !grep( /^$prod_name$/, @exclude_species ) && !-e "$prod_name/alpha_mappings.txt" ) { #Currently does not overwrite existing mappings
				print "Working on: ", $prod_name, "\n";
				mkdir $prod_name unless (-d $prod_name);
				chdir $prod_name;
				`tar xaf $afdb_path/$archive_name *.pdb.gz --to-command="zgrep DBREF" > alpha_mappings.txt`;
				chdir $working_dir;
			}
		}
