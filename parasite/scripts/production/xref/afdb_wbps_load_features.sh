    ...
    if  perl -MProductionMysql -E 'say for ProductionMysql->staging->core_databases(@ARGV)' ""  | grep -q $species;
      ...
      if (( $afdb_pf_count > 0 ));
        then echo "Found ${afdb_pf_count} AlphaFold protein features. Skipping $cdb.";
           echo "Drop existing AlphaFold Protein Features...";
           $PARASITE_STAGING_MYSQL-w $cdb -Ne "DELETE t1 FROM protein_feature t1 JOIN analysis t2 ON t1.analysis_id=t2.analysis_id WHERE t2.logic_name='alphafold_import';";
