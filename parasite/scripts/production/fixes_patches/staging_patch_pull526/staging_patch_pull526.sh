# This script was created to perform this patch: https://github.com/Ensembl/staging-patches/pull/526
# While the xrefs for the WormBase species were correct the display gene descriptions
# had not been set for 4 WBPS species. As a result, users were not able to view the human-readable
# names and the description of the genes.
...
# It then uses the copy_descriptions.sh script to print the SQL commands need to copy
...
cat ${db_correspondance_file} | while read dbname1 dbname2;
  do if [[ $($host2 -Ne "SHOW DATABASES" | grep $dbname2) ]];
    then
      ...
      sh ${WORM_CODE}/parasite/scripts/production/fixes_patches/staging_patch_pull526/copy_descriptions.sh $host1 $dbname1 $host2 $dbname2 >> $patchout/${dbname2}.sql
    else
