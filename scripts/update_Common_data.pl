...
if ($build) {
    ...
    print STDERR "during build so using $ace_dir - ensure that the data you are updating is actually in the database.\n";
}
else {
...
sub write_clones2sv  {   
  ...
  $log->write_to("Updating clone2sv\n");
    my %clone2sv;
...
<<<<<<< update_Common_data.pl
  ...
  while (<TACE>) {
=======
  $log->write_to("Updating clonesize\n");
  my %clonesize;
