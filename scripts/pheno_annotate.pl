...
# Last updated on: $Date: 2003-01-13 16:41:49 $
...
show -a -f    /wormsrv1/geneace/ERICHS_DATA/seq_dump.ace
...
show -a -f    /wormsrv1/geneace/ERICHS_DATA/loci_dump.ace
...
open(IN, "seq_dump.ace") || die "Can,t read in file!";
open(IN, "loci_dump.ace") || die "Can,t read in file!";
open(OUT, ">seq_loci_dump_modified.ace") || die "Can't write file!";

while(<IN>){
  chomp;
  print OUT $_, "\n";
  if( ($_ =~ /^Concise_description.+/)  || 
      ($_ =~ /^Detailed_description.+/) ||
      ($_ =~ /^Provisional_description.+/) ){
    print OUT "-D $_\n";
  }
}  

$command=<<END;
pparse /wormsrv1/geneace/ERICHS_DATA/seq_loci_dump_modified.ace
