...
print "#$species\t# $WS_version\t# $wormbase\t #$yfile_name#\n";
...
print " -user  $config->{core_database}->{user} -dbname $config->{core_database}->{dbname} -host $config->{core_database}->{host} -port $config->{core_database}->{port}  $config->{core_database}->{password}\n";
...
sub copy2acari {
  my ($option) = shift;
  $wormbase->run_script( "BLAST_scripts/copy_files_to_acari2.pl -$option", $log );
  print "BLAST_scripts/copy_files_to_acari.pl $option $log\n";
}


#print "Updated DB $updated_dbs  $current_dbs\n";
...
sub update_blast_dbs {
	print "Starting update_blast_dbs\n";
  ...
  open( OLD_DB, "<$last_build_DBs" ) or die "cant find $last_build_DBs";
   while (<OLD_DB>) {
    ...
    if (/$regexp/) {
        ...
        # If there is a test DBin the mix and warn about that. Only disable this check if you are certain this is what you want
    	if (($_=~/666/ || $_=~/2474/ ) and not $test ) {
		print "ERROR File $last_build_DBs contains DB name $_ containing 665 or 666 which may be test DBs\nPlease go in and correct so that the file is pointing to all the right DBs\n";
		die;
	}
      $_currentDBs{$1} = $_;
      print "Match\t$_\n";
