...
foreach my $gene (@query_genes) {
   ...
   foreach my $ortholog (@orthologs) {
	  ...
	  $genes{$wbgeneid}.="Ortholog $ortho_geneid \"$target_species_long\" From_analysis TreeFam // Ortholog of $query_species ${\$gene->ID} is $target_species ${\$ortholog->ID} bootstrap=${\$ortholog->bootstrap}%\n";
	  $genes{$ortho_geneid}.="Ortholog $wbgeneid \"$query_species_long\" From_analysis TreeFam // Ortholog of $query_species ${\$gene->ID} is $target_species ${\$ortholog->ID} bootstrap=${\$ortholog->bootstrap}%\n";
   }
}
