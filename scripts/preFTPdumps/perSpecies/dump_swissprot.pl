...
my $file = $wormbase->reports . '/'.
   join('.',$wormbase->gspecies_name,$wormbase->ncbi_bioproject,$wormbase->get_wormbase_version,'swissprot.txt');
...
my $iterator = $db1->fetch_many(-query=>"find Protein Species=\"${\$wormbase->long_name}\" AND WormPep AND Live",-filltag=>'Database');
