...
# Last updated on: $Date: 2002-06-19 20:12:14 $
#
...
sub checkgenes {
  ...
  foreach my $child ($obj->Subsequence) {
    ...
    if ($child =~ /\S+\.t\d+/) {
      if (!defined ($child->at('Properties.Transcript'))) {
