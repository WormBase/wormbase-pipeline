...
my $ovlp = Overlap->new($database, $wormbase);
  ...
  if ($gff_directory && $gff_file && $gff_source && $gff_type && $ID_after) {
    ...
    if ($species ne 'sratti' && $species ne 'remanei') {push @est_hsp, $ovlp->get_Trinity_BEST($chromosome)} else {$log->write_to("$species doesn't have Trinity data, so skipping this data\n")} # and the list of Trinity transcripts ...
