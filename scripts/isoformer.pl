...
# Last updated on: $Date: 2014-03-21 17:14:09 $      
...
while (1) {
  ...
  do {
    ...
    if ($userinput eq '?' || $userinput eq 'h' || $userinput eq 'help') { # help
      ...
      print "c, clear, clear all : clear all isoformer objects\n";
    ...
    if ($userinput =~ /^c\s+/ || $userinput =~ /^clear\s+/) {
      &clear($userinput);
      next;
    }
...
sub load_isoformer_method {
  ...
  print OUT "\n\n";
  print OUT "\n\n";
  print OUT "Method : \"non_coding_transcript_isoformer\"\n";
...
sub iterate_through_the_valid_structures {
  ...
  do {
	...
	$$next_isoform++;
      ...
      my $confirmed_ok = &check_not_already_curated($confirmed, $sense, $type, $clone, $clone_aug, $clone_stop, $exons);
      if (!$confirmed_ok) {
	&make_isoform('non_coding_transcript_isoformer', $TSL, $clone, $clone_aug, $clone_stop, $sense, $exons, $$next_isoform, $gene);
	push @created, "non_coding_transcript_isoformer_${$next_isoform}";
	$$next_isoform++;
      }
...
sub check_not_already_curated {
  ...
  foreach my $CDS ( @clone_entry ) {
	...
	$CDS->Method->name ne 'non_coding_transcript_isoformer'
       ) {next} # don't want to report a match to a history object or an ab-initio prediction etc.
    $name = $CDS->name;
...
sub make_isoform {
  ...
  if ($Method eq 'isoformer') {
    $biotype = 'CDS';
    $clone_tag = 'CDS_child';
  } elsif ($Method eq 'non_coding_transcript_isoformer') {
    $biotype = 'Transcript';
...
sub clear {
  ...
  my @CDS_objects = $db->fetch(CDS => "isoformer_*");
  my @ncRNA_objects = $db->fetch(Transcript => "non_coding_transcript_isoformer_*");
  my %todelete;
  ...
  if ($#f > 0 && $f[1] ne 'all') { # if nothing after 'clear' then delete everything
    ...
    foreach my $param (@f) {
      if ($param =~ /^isoformer_\d+$/ || $param =~ /^non_coding_transcript_isoformer_\d+$/) {
	if (exists $todelete{$param}) {
	  $explicitly_delete{$param} = $todelete{$param};
	}

      } elsif ($param =~ /^\d+$/) {
	if (exists $todelete{"isoformer_$param"}) {
	  $explicitly_delete{"isoformer_$param"} = $todelete{"isoformer_$param"};
	}
	if (exists $todelete{"non_coding_transcript_isoformer_$param"}) {
	  $explicitly_delete{"non_coding_transcript_isoformer_$param"} = $todelete{"non_coding_transcript_isoformer_$param"};
	}
      } else {
