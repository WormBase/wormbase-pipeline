...
my ($create_file, $delete_file, $split_file, $merge_file, $update_file, $wb_gff_file, $gff_file, $out_file, $test, $log_file, $species, $debug, $database, $nameserver, $person, $verbose, $start_codons_file);
my @references;
...
sub merge_genes {
	...
	for my $gene_to_merge(@wb_ids) {
	    my $gene = $db->fetch(Gene => $gene_to_merge);
	    if (!$gene) {
...
sub update_gene {
	...
	my $wb_name = $wb_gene->Sequence_name->name;
	my %methods;
	...
	$log->error("Cannot update $wb_id as gene doesn't exist in database\n");
    }
