...
# kj2
#
# maps superlinks into overlapping virtual objects with a size of 150 000 bp
# to be used on a file generated by dumping chromosomes from autoace 
# (see ../blat2/rawdata/chromosomes.ace) to STDOUT
#
# superlinks.blat.pl [-c -m -x -o -e -f] 
#
...
use strict;
use Getopt::Std;
use vars qw($opt_c $opt_s $opt_e $opt_m $opt_x $opt_o $opt_h $opt_f);
...
if ($opt_s) {
    my @stlclones = qw(SUPERLINK_RW1 SUPERLINK_RW1R SUPERLINK_RW2 SUPERLINK_RW2R SUPERLINK_RW3A SUPERLINK_RW3B SUPERLINK_RW4 SUPERLINK_RW5 SUPERLINK_RWXL SUPERLINK_RWXR);
...
while (<ACE>) {
	...
	for ($n = 0; $n <= $total; $n++) {
		...
		$second = $length;
		if ((exists $camace{$name}) || (exists $stlace{$name})) {
		    print "S_Child Homol_data $word{$type}:$name"."_$m $first $second\n";    
		}
		last;
	    }					
	    else {
...
=head2 NAME - superlinks.blat.pl

=head1 USAGE 

=over 4

=item superlinks.blat.pl [-options]

=back

superlinks.blat.pl creates the 150 kb virtual objects used in BLAT mappings. 

superlinks.blat.pl mandatory arguments:
...
superlinks.blat.pl  OPTIONAL arguments:
