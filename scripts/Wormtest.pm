...
=head2 cdnafiles_present()                                                                                                                                                                                 
                                                                                                                                                                                                            
    Function: checks for presence of a bunch of files generated by the autoace_builder.pl -cdnafiles step
...
sub cdnafiles_present {
    my $self = shift;

#       All
#       $BUILD_HOME/BUILD/$SPECIES/SEQUENCES/intergenic_sequences.dna
#	$BUILD_HOME/BUILD/$SPECIES/SEQUENCES/upstream_sequences.dna 
#	$BUILD_HOME/BUILD/$SPECIES/SEQUENCES/mRNA_transcripts.dna
#	$BUILD_HOME/BUILD/$SPECIES/SEQUENCES/pseudogenic_transcripts.dna
#	elegans
#	$BUILD_HOME/BUILD/$SPECIES/SEQUENCES/transposon_transcripts.dna
#	$BUILD_HOME/BUILD/$SPECIES/SEQUENCES/transposons.dna
    my $errors = "0";
    my @filenames = ('intergenic_sequences.dna.gz', 'upstream_sequences.dna','mRNA_transcripts.dna','pseudogenic_transcripts.dna');
    if ($self->{'wormbase'}->species eq 'elegans') {
	push (@filenames,'transposon_transcripts.dna');
	push (@filenames,'transposons.dna'); 
    }
    foreach my $filename (@filenames) {
        if (-e $self->{'wormbase'}->sequences . '/' . $filename) {
            $self->{'log'}->write_to("$filename present, no further action required\n");
        }
        else {
	    $self->{'log'}->write_to("$filename not present, something has gone wrong\n");
	    $errors++;
        } 
    }
    $self->{'log'}->write_to("All expected cdna output files found\n") unless $errors;
    return $errors;
}
...
sub map_present {
    ...
    #files
    # $BUILD_HOME/BUILD/autoace/acefiles/PCR_products_to_genes.ace
    # $BUILD_HOME/BUILD/autoace/acefiles/Oligo_sets_to_genes.ace
    # $BUILD_HOME/BUILD/autoace/acefiles/microarray_mappings.ace
    # $BUILD_HOME/BUILD/$SPECIES/attach_other_nematode_ests.ace
    # $BUILD_HOME/BUILD/$SPECIES/acefiles/TSL_feature_connections.ace	
    # elegans
    # $BUILD_HOME/BUILD/autoace/acefiles/map_tags.ace
    # $BUILD_HOME/BUILD/autoace/acefiles/Interaction_connections.ace
    # $BUILD_HOME/BUILD/autoace/acefiles/RNAi_mappings.ace


    my $errors = "0";
    my @filenames = ('Oligo_sets_to_genes.ace', 'microarray_mappings.ace','TSL_feature_connections.ace','attach_other_nematode_ests.ace');
    if ($self->{'wormbase'}->species eq 'elegans') {
        push (@filenames,'map_tags.ace');
        push (@filenames,'Interaction_connections.ace');
	push (@filenames,'RNAi_mappings.ace');
    }
    foreach my $filename (@filenames) {
        if (-e $self->{'wormbase'}->acefiles . '/' . $filename) {
            $self->{'log'}->write_to("$filename present, no further action required\n");
        }
        else {
            $self->{'log'}->write_to("$filename not present, something has gone wrong\n");
    ...
    $self->{'log'}->write_to("All expected map output files found\n") unless $errors;
    return $errors;
}
