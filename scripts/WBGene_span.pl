...
if ( $prepare ) {
  ...
  &run_command("perl $ENV{'CVS_DIR'}/GFF_method_dump.pl -database $database -method Coding_transcript -dump_dir ".$wormbase->gff_splits);
  ...
  foreach my $chrom ( @chromosomes ) {
    my %gene_coords;
    foreach my $method (qw(Coding_transcript Non_coding_transcrip tRNAscan-SE-1.23 miRNA) ) {
