...
# Adapted to appraise each C. elegans gene 
#
...
###########################################################
#find protein with which to do protein comparison;
###########################################################

($protein,$score)=&tophit($ARGV[0]);

if ($protein=~s/^TR://) {$database="SPTREMBL";}
if ($protein=~s/^SW://) {$database="SWISSPROT";}
if ($protein=~s/^WP://) {$database="WORMPEP";}

print STDERR "looking for Protein  $protein - has score $score\n";
unlink "/tmp/protein$$";
if ($database eq "SPTREMBL" || $database eq "SWISSPROT") {system("getz5  -d -sf fasta   \"[$database-id:$protein]\" > /tmp/protein$$");}
if ($database eq "WORMPEP") {system("getz5  -e  \"[$database-acc:$protein]\" > /tmp/protein$$");}

############################################################
#make the DNA sequence with which to do the comparison
############################################################

$link=&getlink($ARGV[0]);
print "LINK is $link\n";
#find the start and end of the gene in this link
($start,$end)=&getends($link,$ARGV[0]);
print "Gene $ARGV[0] starts at $start and ends at $end\n";

exit;
#make a file containing the DNA of the gene region 5000 upstream and downstream
&makeDNA($ARGV[0],1000,"/tmp/tempdna$$");
#&makeGFF($ARGV[0],1000,"/tmp/tempgff$$");
...
#&Wise2::change_max_BaseMatrix_kbytes(2000000); # 2 Gigabytes....heavy....
...
open(textace, "echo '$command' | $exec | ");
    while(<textace>) {if (/^Source\s+\"(\S+)\"/) {$link=$1;}
                     }
    close textace; 
...
#####################################################################
#get the ends of a gene in a link
#####################################################################
...
EOF
    open(textace, "echo '$command' | $exec  |");
while (<textace>) {if (/^\S+\s+Coding\s+sequence\s+(\d+)\s+(\d+)\s+\S+\s+\S+\s+\S+\s+(\S+)/) 
		   {if ($3 eq $_[1]) {print "here $1 $2\n";$start=$1;$end=$2;}}
		    }
    return ($start,$end);
