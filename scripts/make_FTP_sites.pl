...
my (%skip_species, @skip_species, @only_species, %only_species, $WS_version, $WS_version_name, $agr_blast_meta);
...
&make_blast_meta         if $agr_blast_meta or $all;
...
sub copy_ontology_files {
  ...
  print "Copying obo files $obo_dir\n";
...
=pod
  ...
  foreach my $wb (values %accessors) {
    ...
    my $in_file = $wormbase->autoace . "/ONTOLOGY/gene_association.${WS_version_name}.wb.$gspecies";
    my $out_file = "$targetdir/species/$gspecies/$bioproj/annotation/$gspecies.${bioproj}.${WS_version_name}.go_annotations.gaf.gz";

    if (-e $in_file) {
        $wormbase->run_command("cat $in_file | gzip -n -9 -c > $out_file", $log);
    }else{
        $log->write_to("WARNING: cannot find $in_file\n");
    }
  }
=cut
...
sub make_blast_meta{
	...
	my $genome_path = $fasta_dir . '/' . $genome_file;
