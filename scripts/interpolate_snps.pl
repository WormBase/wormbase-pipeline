...
foreach my $i qw( I II III IV V X ) {
    $m->submit( -w => "done($myid)", "interpolate_gff.pl -chrom $i $flags -snp" );
}
...
if ( !$noload ) {
    ...
    foreach my $file ( glob("$acedir/interpolated_SNP_*.ace") ) {
        $wb->load_to_database( $wb->autoace, $file, "interpolate_snps", $log );
    }
}
$log->mail();
