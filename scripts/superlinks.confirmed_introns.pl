...
#		if ($length <= 100000) {
#			if ((!$opt_c) || (exists $camace{$name})) {
#				print "S_Child Homol_data $name"."_1 1 $length\n\n"; 
#			}
#		}
#		else {
			...
			for (my $n = 0; $n <= $total; $n++) {
				...
				if (($length - $first) < 100000) {
					$second = $length;
					if ((!$opt_c) || (exists $camace{$name})) {
						print "S_Child Feature_data Confirmed_intron_EST:$name"."_$m $first $second\n" unless ($opt_m);  
						print "S_Child Feature_data Confirmed_intron_mRNA:$name"."_$m $first $second\n"    if ($opt_m);  
					}
					last;
				}					
#				elsif ($second > $length) {
#					$second = $length;
#					if ((!$opt_c) || (exists $camace{$name})) {
#						print "S_Child Homol_data $name"."_$m $first $second\n";  
#					}
#				}	
				else {
					if ($second >  $length) {
