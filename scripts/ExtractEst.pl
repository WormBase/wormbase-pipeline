#!/usr/local/bin/perl
# ExtractEst reads EST matches and genes from subsets of
# dumped gff C.Elegans chromosome files and calculates the
# correlation between them.
# Needs as input a subset of chromosome files containing EST matches
# (grep EST_GENOME) and a subset containing only sequences
# (grep sequences + editing).
# Ver. 1 [ag3 rbsk 221199]

BEGIN {
 unshift (@INC,"/nfs/disk65/ag3/cgi-bin/GFF-2.12");
...
trace("Group tag name for file 1 :\t\t$tag1\n") ;

my $tag2 = defined($opt_H) ? $opt_H : undef ;
trace("Group tag name for file 2 :\t\t$tag2\n") ;
...
GFF::trace(1) if $opt_T or $opt_D ;
# any overlap, strand insensitive, report overlaps to STDERR
#my $gffo = $gff1->intersect_overlap_matches( $gff2,-1,0,0,1,\*STDERR) ; 
my $gffo = $gff1->intersect_overlap( $gff2,-1,0,0,1,\*STDERR) ; 
GFF::trace(0) if $opt_T or $opt_D ;

############### Retrieve the matches ############

print "*** Here we have the list of matches ***\n\n";
$gffo->dump_matches(\*STDERR);
...
# Insert here if diff matches calculate lenght between them
# 
