...
Usage: $0 <options> domain id nametype1=name1 nametype2=name2...
 Add one or more names to the indicated id
...
Options can be abbreviated, as in:
  ...
  id_addname.pl -d mysql:test -u fred -p secret Gene P0000001 cgc=unc-3 wtp=3j45
END
...
while (my $name = shift) {
  ...
  $db->addName($id,$1=>$2) or die "$name: couldn't add\n";
}
