...
my ($help, $debug, $test, $store, $database, $outfile);
...
if (!defined $outfile) {
    $outfile = "StrainVariationNSdiscrepancies.ace";
}
...
my $def = "$acedb/wquery/SCRIPT:geneace_nameDB_Straincomm.def";
...
while( <$TABLE> ){
	...
	unless ($status) {
	    $errorcount++;
	    print "This Strain has no Status $var\n";
	}
...
my $PASS    = 'wormpub';
my $USER    = 'wormpub';
my $DOMAIN  = 'StrainVariation';
my $db = NameDB_handler->new($DB,$USER,$PASS);

# get nameserver data
my $query = 'SELECT pi.object_public_id, pi.object_live, si.object_name FROM   primary_identifier pi,secondary_identifier si WHERE  pi.object_id = si.object_id AND pi.domain_id = 4';
...
#is it also in the nameserver
	if( $server_vars{"$var"} ){
		...
		$errorcount++;
		print "ERROR: $var live or dead ? ns:$nsstatus g:$acestatus\n $var\n";
		return;
	    }
...
#confirm both have them
	    ...
	    $errorcount++;
	    print "ERROR: $var missing from server\n[errror 7] $var\n";
	    return;
	}	
    }
    else {
