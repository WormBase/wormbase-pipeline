...
# get the species object from the list of accessors
  ...
  chdir "/lustre/scratch101/ensembl/wormpipe/sort_dump" || die "Can't cd to /lustre/scratch101/ensembl/wormpipe/sort_dump: $!";
  ...
  if ($available < 30000000) { # NB df returns the available space in 1Kb blocks
    $log->error("\nThere is less than 30Gb of free disk space available on /lustre/scratch101/ensembl/wormpipe/sort_dump\nAborting - not running $species\n");
    next;
  }
  ...
  $wormbase->run_command('rm -f /lustre/scratch101/ensembl/wormpipe/sort_dump/junk*', $log);
  ...
  &wait_for_prompt($exp);
  $exp->send("cd /lustre/scratch101/ensembl/wormpipe/sort_dump\n");
  &wait_for_prompt($exp);
  ...
  $exp->send("$cmd\n");
  &wait_for_prompt($exp);
  $exp->send("rm -f /lustre/scratch101/ensembl/wormpipe/sort_dump/junk*.*\n");
  &wait_for_prompt($exp);
