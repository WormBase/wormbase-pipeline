...
# create output files
open (LOG, ">blastx_dump.log") || die "cannot create log file";
open (ACE, ">blastx_dump.ace") || die "cannot create ace file";
#*LOG = *STDOUT;
...
if ($opt_m) {
    print LOG "get the accession 2 name mapping for the AceDB contigs [".&now."]\n\n";
...
#open (DB , "accession2name") || die "cannot open file";
#while (<DB>) {
#  chomp;
#  my ($accession, $name) = split /\s+/;
#  $accession2name{$accession} = $name;
#}
#close DB;
