...
#       new forms in stlace not yet appearing in acedb 
...
#  otherwise, all of stlace is checked
#
#  - connects to stlace for various data
...
# open outfile
my $cwd = getcwd();
my $e_upload_dir = "/nfs/wormpub/PAD/";

unless ($cwd eq $e_upload_dir) {
    chdir($e_upload_dir) or die "\n\nFAILED to change to '$e_upload_dir' : $OS_ERROR\n\n";
}

#Todays date and display like this: 20091215
my $dt = DateTime->now();
$dt->set_time_zone( 'Europe/London' );
my $Date = $dt->ymd('');

open(OUT, ">pfam_remark.".$Date.".ace");
$cwd = getcwd();
...
# database paths
my %db_list = (
	    'acedb',   "/nfs/wormpub/DATABASES/current_DB",  
            'stlace',  "/nfs/wormpub/DATABASES/camace",
	);
...
my $cur_acedb64b_tace = "/software/worm/bin/acedb/tace";
my $db = Ace->connect (-path => "$db_path",
		       -program => "$cur_acedb64b_tace") || 
...
if($ARGV[0]) {
    ...
    @sequences = $db->fetch(-query => 'Find CDS *.* Species="Caenorhabditis elegans" & Method!=history');
...
# get data from stlace

    my $obj = $db->fetch(CDS => $gene);
#    $locus = $obj->Locus(1);
    my $ggene = $obj->Gene(1);
    if($ggene) {
	my $ggeneobj = $db->fetch(Gene => $ggene);
	$locus = $ggeneobj->CGC_name(1);
    }
...
# sort score hash by value
	for (sort { $pepscore{$b} <=> $pepscore{$a} } keys %pepscore) {

#	    print "looking at $_ : score = $pepscore{$_}\n";
...
PRINTIT:
    ...
    print OUT "CDS : $gene\n";
    print OUT "-D DB_remark\n";
    print OUT "DB_remark \"$full_string\"\n\n";
...
#    $locus = $obj->Locus(1);
	...
	if($thismethod eq 'non_coding_transcript') {
	    if ($locus) { $full_string .= "C. elegans non-functional isoform from coding locus $locus"; }
	    else { $full_string .= "C. elegans non-functional isoform from coding locus"; }
	}
	elsif($thismethod eq 'ncRNA') {
	    if ($locus) { $full_string .= "C. elegans probable non-coding RNA $locus"; }
    ...
    elsif($transcript1 eq 'piRNA') { # piRNA genes
      if ($locus) { $full_string .= "C. elegans piwi interacting RNA $locus"; }
      else { $full_string .= "C. elegans predicted piwi interacting RNA"; }
    }
    elsif ($transcript1 eq 'lincRNA') { # lincRNA genes
      if ($locus) { $full_string .= "C. elegans large intervening non-coding RNA $locus";} 
      else {$full_string .= "C. elegans predicted large intervening non-coding RNA ";}
    }
    elsif ($transcript1 eq 'asRNA') { # asRNA genes
      if ($locus) { $full_string .= "C. elegans antisense RNA $locus";} 
      else {$full_string .= "C. elegans predicted antisense RNA ";}
    }
