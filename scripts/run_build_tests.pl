...
GetOptions(
    ...
    'blastx'             => \$blastx, # compares counts of BLASTX entries in the generated ace file against previous build
    'blastp'             => \$blastp, # compares counts of BLASTP entries in the generated ace file against previous build
...
if ($blastx) {
    $errors += $wormtest->blastx_counts_comparison;
}
if ($blastp) {
    $errors += $wormtest->blastp_counts_comparison;
}
if ($dbxref) {
...
if ($errors) {
    $log->log_and_die("\n**** WARNING: $errors errors identified by run_build_tests.pl ****\n");
}
else {
    $log->write_to("\nAll build tests passed, no errors identified\n");
} 
