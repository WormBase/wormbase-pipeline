...
INSERT IGNORE INTO object_xref (ensembl_object_type,ensembl_id,xref_id,linkage_annotation,analysis_id) SELECT 'Probe', probe_id,xref_id,'[imported from WormBase]',5 FROM probe join probe_set p using (probe_set_id), xref x  WHERE x.dbprimary_acc like "#{hit}%" AND p.name="#{array_name}";
