...
my $operon_analysis = $analysis_adaptor->fetch_by_logic_name($WB_RNAI_LOGIC_NAME);
...
foreach my $chromosome_info(@{$WB_CHR_INFO}) {
  ...
  print "reading ",$chromosome_info->{'gff_file'},"\n";
  
  my @operons = @{&parse_rnai($chromosome_info->{'gff_file'}, $chr, $operon_analysis)};
