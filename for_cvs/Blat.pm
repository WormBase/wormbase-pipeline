...
sub run {
  ...
  my $results = parse_results($self,$blat_output_pipe);
...
sub parse_results {
    ...
    my @alignments;
    my $slice_adaptor=$self->analysis->adaptor->db->get_SliceAdaptor();
	...
	my @pairs; # later to generate a sane cigar string
	...
	for (my $i=0; $i<$block_count; $i++ ) {
	    ...
	    #				     -id => "q_seq");
            ...
            my $percent_id   = sprintf "%.2f", ( 100 * ($matches+$rep_matches) / ( $matches + $mismatches + $rep_matches ) );
				      ...
				      #my ($score, $percent_id, $frame) = get_best_score_in_all_frames($this_q_bioseq, $this_t_bioseq);
	    ...
	    my $q_strand = ($feat2{strand} eq '-')?"-1":"1";
            ...
            # if q_strand -1 then reverse both .... no clue if it works with --/+-/-+
	    if ($q_strand == -1){
		    $t_strand=$t_strand*-1;
		    $q_strand=$q_strand*-1;
	    }
	    ...
	    $feature_pair->hseqname($feat2{name});#.':'.$digest);
	    $feature_pair->hstart($query_starts[$i]);
	    $feature_pair->hend($query_ends[$i]);
	    $feature_pair->hstrand($q_strand);
   
	    $feature_pair->analysis($self->analysis);
	    $feature_pair->slice($slice_adaptor->fetch_by_region( 'chromosome', $feat1{name} ));
	    ...
	    push(@pairs,$feature_pair);
        }

        my $alignment = new Bio::EnsEMBL::DnaDnaAlignFeature(-features => \@pairs);
        push @alignments, $alignment;
    }
